...
export default function PortMappings() {
      ...
      return (
        <dl>
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-lg font-bold text-gray-500 text-center">
              {mapping.port}
            </dt>
  ...
  const renderRows = useMemo(
    () => (portMappings: EnrichedPortMappings) => {
      const portRows = [];
      const orderedMappings = Object.keys(portMappings).sort(function (a, b) {
      ...
      for (const portKey of orderedMappings) {
        portRows.push(renderRow(portMappings[portKey]));
      }
      return portRows;
    },
    []
      ...
      <div className="bg-gray-100 shadow overflow-hidden sm:rounded-lg my-4">
        <div className="border-b-2 border-gray-200 bg-gray-100 text-xs font-semibold text-gray-600 uppercase tracking-wider px-4 py-5 sm:px-6">
          <div className="flex flex-row">
            <h3>
              Ports can be mapped to names that allow you to easily identify
              them and make tunnel configuration's more definable.
            </h3>
