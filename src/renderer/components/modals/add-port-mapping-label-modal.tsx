...
import { addLabelsToPortMapping, createPortMapping } from "../../state/slices/portMappings";
...
export default function AddPortMappingLabelModal(props: AddPortMappingLabelModalProps) {
	...
	return (
		<ModalBase
			...
			openModalButton={<IconButton title="Add Label to Port" onClick={() => setOpen(true)} />}
			open={open}
			actions={[
				<IconButton
					title="Submit"
					...
					onClick={() => {
						...
						addLabelsToPortMapping(dispatch, share, Number(props.mapping.port), labels);
						clearModal();
					}}
				/>,
				<IconButton
			...
			titleIcon={<PencilIcon className="h-6 w-6 text-gray-500" aria-hidden="true" />}
		/>
	);
}
