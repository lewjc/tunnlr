...
const renderPagination = (selectedPage: number, totalPages: number, onPageChanged: any) => {
	const pageNumbers = [];
		...
		return (
			...
			>
				<svg
					...
					className={`feather ${up ? "feather-chevron-right" : "feather-chevron-left"} w-4 h-4`}
				>
					{up ? (
	...
	return (
		<div className="flex flex-col items-center my-4">
			<div className="flex">
				{renderArrow(false)}
				<div className="flex h-8 font-medium rounded-full bg-gray-100">{pageNumbers}</div>
				{renderArrow(true)}
			</div>
		</div>
	);
		...
		<>
			...
			<div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
				<div className="inline-block min-w-full rounded-lg overflow-hidden">
					...
					{renderPagination(pageNumber, Object.keys(enrichedHosts).length, setPageNumber)}
				</div>
			</div>
		</>
	);
