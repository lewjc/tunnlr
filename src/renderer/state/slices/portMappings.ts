...
export const portMappingsSlice = createSlice({
  ...
  reducers: {
    ...
    setPortMappings: (
      state,
      action: PayloadAction<PortMappingsState["portMappings"]>
    ) => {
    ...
    addPortMappingLabels: (state, action: PayloadAction<PortMapping>) => {
      ...
      state.portMappings.mappings = [
        ...state.portMappings.mappings.filter(
          (x) => x.port !== action.payload.port
        ),
        action.payload,
      ];
      console.log(action.payload);
...
export const createPortMapping = async (
  ...
  try {
    ...
    ipcRenderer.on(
      definition.addPortMapping.response,
      (event, addedMapping) => {
...
export const addLabelsToPortMapping = async (
  ...
  try {
    ...
    ipcRenderer.on(
      definition.addPortMappingLabels.response,
      (event, addedMapping) => {
