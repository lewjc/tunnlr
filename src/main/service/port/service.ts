...
const addPortMappingLabels = async (evt: IpcMainEvent, port: number, labels: Array<string>) => {
	console.log(port);
	console.log(labels);
	const portMappings: PortMappingConfig = readPortMappingsFromFile();
...
export interface PortMappingsService {
	...
	functions: ServiceFunctions<PortMappingFunctions>;
}
...
export interface PortMappingFunctions {
	getPortMappings: IpcMain;
	addPortMapping: IpcMain;
	addPortMappingLabels: IpcMain;
}
...
export default {
	...
	functions: {
		getPortMappings: ipcMain.on(getPortMappingsEvents.send, getPortMappings),
		addPortMapping: ipcMain.on(addPortMappingEvents.send, addPortMapping),
		addPortMappingLabels: ipcMain.on(addPortMappingLabelsEvents.send, addPortMappingLabels),
	},
};
